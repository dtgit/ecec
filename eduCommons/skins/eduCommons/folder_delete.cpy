...
for path in paths:
	...
	if not contains_published:
	    if obj.portal_workflow.getInfoFor(obj,'review_state') == 'Published':
...
if titles:
    ...
    context.restrictedTraverse('@@search_view').notifyDeleteObjectEvent(context,bulkChange=True,contains_published=contains_published)
